...
int main()
{
	...
	if (!udp_init(&comunication1, "127.0.0.1", 13331, "127.0.0.1", 13332, UDP_BUFFER_SIZE, UDP_BUFFER_SIZE)) {
	...
	int send_index = 0, received_index = 0;
	long send_data = 0, received_data = 0;
	for (int i = 0; i < 10000; ++ i) {
		...
		printf("Remote: received index: %d, data: %ld, sent index: %d, data: %ld\n", received_index, received_data, send_index, send_data);
		sleep_usec(2000);
	}
